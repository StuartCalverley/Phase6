...
public class ValidatorTest {
	...
	public void matchNumNameChecker(){
		...
		assertEquals("Account name and number match.", true, validator.matchNumName(accounts, 10000, "Jake"));
		assertEquals("Account name and number do not match.", false, validator.matchNumName(accounts, 10001, "Jake"));
	}
	
	@Test
	public void verifyUserChecker(){
		Validator validator = new Validator();
		Map<Integer, BankAccount> accounts = new HashMap<Integer, BankAccount>();
		accounts.put(10000, new BankAccount(10000, "Jake", true, 100.00f, 'N', 5));
		accounts.put(10002, new BankAccount(10002, "Tom", true, 1000.00f, 'S', 2));
		
		assertEquals("Account name not found.", "Fail", validator.verifyUser(accounts, 10000, "Phil", "Withdrawal"));
		assertEquals("Account number not found.", "Fail", validator.verifyUser(accounts, 10001, "Jake", "Withdrawal"));
		assertEquals("Account name and number do not match.", "Fatal", validator.verifyUser(accounts, 10000, "Tom", "Withdrawal"));
		assertEquals("Account name and number valid.", "Pass", validator.verifyUser(accounts, 10000, "Jake", "Withdrawal"));
